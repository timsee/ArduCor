...
* `pi` contains a python script intended for a Raspberry Pi that takes wireless packets and sends them over serial to an arduino.

For documentation for the Yun based samples, [check here](yun).

For a guide on how to use the Raspberry Pi sample, [check here](pi).
